...
class Pix2PixLiveInputTestModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if not self.isTrain:
            self.visual_names = ['input', 'seg_fB0', 'seg_fB1', 'fake_B', 'real_A', 'decision_map', 'S']
            self.isValid = False
    ...
    def set_input(self, input):
        ...
        #self.S[self.decision_map == -1] = -1#-1 #### This is 0 in auto_region_select_multiregion!
        # Concatenate confocal and STED_map to two channels input
        self.input = torch.cat((self.real_A, self.S, self.decision_map), 1)
        print(self.real_A.min(), self.real_A.max(), self.S.min(), self.S.max(), self.decision_map.min(), self.decision_map.max())
