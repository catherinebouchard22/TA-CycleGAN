...
class CycleTranslatorModel(BaseModel):
    """
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        parser.set_defaults(norm='batch', netG='resnet_9blocks', netS='resnet_6blocks', dataset_mode='fixedlive_classifier', preprocess='crop_rotation', crop_size=224, batch_size=1)
        if is_train:
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.visual_names = ['fake_L', 'GT_labels']
        ...
        net_params = load('checkpoints/UNet_Dendrite', True)
        trainer_params = pickle.load(open(os.path.join('checkpoints/UNet_Dendrite', "params_trainer.pkl"), "rb"))
    ...
    def set_input(self, input):
        ...
        if not self.isTrain:
            self.GT_labels = input['GT_labels']
    ...
    def forward(self):
        ...
        #self.real_F_seg = self.real_F - self.real_F.min()
        #self.real_F_seg /= (0.8 * (self.real_F_seg.max() - self.real_F_seg.min()))
        ...
        if not self.isTrain:
            pass
            #self.seg_F = self.netS(self.real_F).detach() # S_B(B)
            #self.seg_fake_F = self.netS(self.fake_F)
            #self.seg_real_L = self.netS(self.real_L)
