...
class Pix2PixPretrainedSegModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            ...
            self.netV = models.vgg16(pretrained=True).features[:22].to(self.opt.gpu_ids[0])
    ...
    def forward(self):
        ...
        if self.isTrain:
            ...
            self.loss_D = (self.loss_D_fake + self.loss_D_real) * 0.57
            ...
            # Second, G(A) = B
            # "Perceptual" (MSE) loss
            self.loss_G_GEN = self.criterionMSE(self.real_B, self.fake_B) * self.opt.lambda_MSE

            # Added segmentation loss
            self.loss_S_fake = self.criterionSEG(self.seg_fB, self.seg_rB) * self.opt.lambda_seg # loss between fake B segmentation and GT
            self.loss_S_real = self.criterionSEG(self.seg_rB, self.seg_GT) * self.opt.lambda_seg

            # combine loss and calculate gradients
            self.loss_G = self.loss_G_GAN + self.loss_G_GEN +  self.loss_S_fake
