...
class FixedLiveClassifierDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        F = F_transform(F)
        L = L_transform(L)
        GT_rings = F_transform(GT_rings)
        GT_fibers = F_transform(GT_fibers)

        GT = numpy.concatenate([GT_rings, GT_fibers], 0)

        return {'F': F, 'L': L, 'F_paths': F_path, 'L_paths': L_path, 'GT_labels': GT}
