...
class Livestedpairednewpleaseworkdataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        _, h, w = AB.shape
        conf = AB[0]
        conf = conf / conf.max() * 255
        sted = AB[1]
        sup_px_map = AB[2]
        A = Image.fromarray(conf)
        B = Image.fromarray(sted)
        ...
        sup_px_map = A_transform(sup_px_map).type(torch.FloatTensor)
