...
class TwoMasksDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #ABC = tifffile.imread(ABC_path)[0]
        ABC = numpy.array(Image.open(ABC_path)) / 128
        # split AB image into A and B
        h, w = ABC.shape
        w2 = int(w / 4)

        A = Image.fromarray(ABC[:, :w2])
        B = Image.fromarray(ABC[:, w2:2*w2])
        C = Image.fromarray(ABC[:, 2*w2:3*w2])
        D = Image.fromarray(ABC[:, 3*w2:])
