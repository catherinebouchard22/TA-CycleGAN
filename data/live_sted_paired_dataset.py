...
class LiveSTEDPairedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        n = 2#random.choice(range(0,int(0.5*len(sorted_list)+1)))#int(random.uniform(0,0.5) * len(sorted_list))
        sup_px_map = sup_px_map > sorted_list[-(n + 1)]

        # Define region of interest
        #px = 100
        #sup_px_map[int((h-px)/2):int((h+px)/2),int((w2-px)/2):int((w2+px)/2)] = 0
