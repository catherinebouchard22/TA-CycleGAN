...
class LiveSTEDSeriesTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        AB1 = tifffile.imread(AB_path.replace('STED_2','STED_1'))
        AB0 = tifffile.imread(AB_path.replace('STED_2','STED_0'))
        # split AB image into A and B
        CONF2 = Image.fromarray(AB2[0])
        STED2 = Image.fromarray(AB2[1])
        CONF1 = Image.fromarray(AB1[0])
        STED1 = Image.fromarray(AB1[1])
        CONF0 = Image.fromarray(AB0[0])
        STED0 = Image.fromarray(AB0[1])
        ...
        px = 100 # pixel size
        sup_px_map0 = skimage.measure.block_reduce(STED0.numpy()[0], (px,px), numpy.mean)
        sup_px_map1 = skimage.measure.block_reduce(STED1.numpy()[0], (px,px), numpy.mean)
        sup_px_map2 = skimage.measure.block_reduce(STED2.numpy()[0], (px,px), numpy.mean)

        STED_map = sup_px_map0.copy() * 0 # this map combines all STED_crops
        decision_map = sup_px_map0.copy() * 0 # this map wil give ponderation based on time elapsed since STED acquisition
        decision_map[sup_px_map2 == sup_px_map2.max()] = 1
        decision_map[sup_px_map1 == sup_px_map1.max()] = 0.6
        decision_map[sup_px_map0 == sup_px_map0.max()] = 0.2
