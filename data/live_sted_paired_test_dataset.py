...
class LiveSTEDPairedTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        px = 100#int(numpy.sqrt(0.1*numpy.size(A))) # pixel size
        ...
        sup_px_map = A.numpy()[0] * 0
        sup_px_map[int((h-px)/2):int((h+px)/2),int((w2-px)/2):int((w2+px)/2)] = 1
        sup_px_map = Image.fromarray(sup_px_map)
        tf = transforms.ToTensor()
        sup_px_map = tf(sup_px_map)
