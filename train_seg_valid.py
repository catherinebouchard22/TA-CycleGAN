...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        train_rings, train_fibers, val_rings, val_fibers  = [], [], [], []
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            train_rings.append(losses['S_rings'])
            train_fibers.append(losses['S_fibers'])
        ...
        for i, data in enumerate(dataval):
            ...
            with torch.no_grad():
                ...
                losses = model.get_current_losses()
            val_rings.append(losses['S_rings'])
            val_fibers.append(losses['S_fibers'])
        ...
        if sum(val_rings)/len(val_rings)+sum(val_fibers)/len(val_fibers) < best_loss:           
            print('saving the model at the end of epoch %d since the validation loss is at a minimum' % (epoch))
            model.save_networks('best')
            best_loss = sum(val_rings)/len(val_rings)+sum(val_fibers)/len(val_fibers)

        row = [epoch, sum(train_rings)/len(train_rings), sum(train_fibers)/len(train_fibers), sum(val_rings)/len(val_rings), sum(val_fibers)/len(val_fibers)]
        with open(f,'a') as file:
            writer = csv.writer(file)
            writer.writerow(row)
